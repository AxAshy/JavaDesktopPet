...
import com.group_finity.mascot.platform.jna.Dwmapi;
import com.group_finity.mascot.platform.jna.User32Extra;
...
/**
 * 使用 JNI 来获取环境信息对应Java来说是比较困难的
 */
public class WindowsEnvironment extends Environment{
    private static final HashMap<HWND, Boolean> ieCache = new LinkedHashMap<>(); // 使用一个哈希表存储窗口句柄的状态
    ...
    private enum IeStatus {
        /** 当前 IE 是有效的. */
        VALID,
        /** 当前 IE 是无效的，并妨碍了其他的有效 IEs. */
        INVALID,
        /** 当前 IE 是无效的，但是不会妨碍其他 IEs 变成有效状态. */
        IGNORED,
        /** 当前 IE 超出了边界，但是不会妨碍其他 IEs 变成有效状态. */
        OUT_OF_BOUNDS
    }
    ...
    /**
     * HWND是Windows操作系统中用于唯一标识窗口对象的“窗口句柄”，该方法用于判断给定的{@code hWnd}是否是IE
     * @param hWnd Windows操作系统中用于唯一标识窗口对象的“窗口句柄”
     * @return boolean
     */
    ...
    private static IeStatus getIeStatus(HWND hWnd) {
        if (User32.INSTANCE.IsWindowVisible(hWnd)) {
            // win7和更早的版本不支持DWMWA_CLOAKED , 所有需要确保程序在至少win8及之后的系统上运行
